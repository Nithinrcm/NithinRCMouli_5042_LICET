...
def display(request):
    ...
    return render(request, 'display.html', {'songs': songs, 'user' : username})

def listen_later_playlist(request):
    listen_later_songs = request.user.listen_later_songs.all()
    return render(request, 'listen_later.html', {'listen_later_songs': listen_later_songs})
...
def add_to_listen_later(request, song_id):
    ...
    return redirect('music:listen_later')
